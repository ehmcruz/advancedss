...
void BX_CPU_C::FPU_update_last_instruction(bxInstruction_c *i)
{
  ...
  BX_CPU_THIS_PTR the_i387.fcs = 0; //BX_CPU_THIS_PTR sregs[BX_SEG_REG_CS].selector.value;
  BX_CPU_THIS_PTR the_i387.fip = BX_CPU_THIS_PTR prev_rip;

  if (! i->modC0()) {
     BX_CPU_THIS_PTR the_i387.fds = 0; //BX_CPU_THIS_PTR sregs[i->seg()].selector.value;
